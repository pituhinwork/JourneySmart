...
export default class MaterialSwitch extends React.Component {
   static defaultProps = {
      active: false,
      disabled: false,
      styles: {},
   ...
   constructor(props) {
      ...
      var w = this.props.switchWidth - Math.min(this.props.
         switchHeight, this.props.buttonRadius*2);
  ...
  changeState(state) {
    var callHandlers = (this.start.stateChanged ?
      this.start.state : this.state.state) != state;
    setTimeout(() => {
  ...
  render() {
    ...
    return (
      <View style={{padding: this.padding, position: 'relative'}}>
        <View style={{
            backgroundColor: this.state.state && !this.props.disabled ?
               this.props.activeBackgroundColor : this.props.inactiveBackgroundColor,
        ...
        <TouchableHighlight underlayColor='transparent' activeOpacity={1} onPress={() => { this.toggle(); }} style={{
          ...
          <Animated.View style={[{
              backgroundColor:
                this.state.state && !this.props.disabled
