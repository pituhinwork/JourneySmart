...
export default class ProgressBar extends React.Component {
  ...
  static propTypes = {
    progress: props =>
      (props.progress < 0 || props.progress > 1
        ? new Error(
          "'progress' supplied to 'ProgressBar' is expected " +
              'to be in the range 0..1 inclusive.',
        )
        : undefined),
  };
  ...
  render() {
    ...
    const innerBorderRadius = this.props.borderRadius - (this.props.borderRadius > 2 ? 2 : 0);
    ...
    if (row) {
      progressStyle[`margin${reverse ? 'Right' : 'Left'}`] = this.props.borderThickness;

      progressStyle[`borderTop${reverse ? 'Right' : 'Left'}Radius`] = innerBorderRadius;

      progressStyle[`borderBottom${reverse ? 'Right' : 'Left'}Radius`] = innerBorderRadius;
    } else {
      progressStyle[`margin${reverse ? 'Bottom' : 'Top'}`] = this.props.borderThickness;

      progressStyle[`border${reverse ? 'Bottom' : 'Top'}LeftRadius`] = innerBorderRadius;

      progressStyle[`border${reverse ? 'Bottom' : 'Top'}RightRadius`] = innerBorderRadius;
    }
